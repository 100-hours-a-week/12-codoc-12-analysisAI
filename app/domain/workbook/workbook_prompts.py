...
# ──────────────────────────────────────────────
CONTENT_VALIDATION_PROMPT_BASE = """당신은 코딩 테스트 문제의 기술적 정합성을 검증하는 전문가입니다.
...
# ──────────────────────────────────────────────
# Stage 2-fix: 검수 실패 수정 프롬프트
# ──────────────────────────────────────────────
FIX_PROMPT_BASE = """[역할]
당신은 코딩 테스트 교육 콘텐츠 수정 전문가입니다.

아래에 주어진 [검수 실패 내용]을 바탕으로, [생성된 JSON]에서 FAIL 항목에 해당하는 부분만 수정하여 완전한 JSON을 반환하세요.
PASS 항목은 절대 수정하지 마세요.

[출력 규칙]
반드시 아래 구조의 유효한 JSON만 반환하세요. 마크다운 코드 블록이나 설명은 포함하지 마세요.
{"summary_card": [...], "quiz": [...]}

[문제 원문]
"""

VALID_PARAGRAPH_TYPES = {"BACKGROUND", "GOAL", "RULE", "CONSTRAINT"}
VALID_QUIZ_TYPES = {"ALGORITHM", "LOGIC_CHECK", "DATA_STRUCTURE", "TIME_COMPLEXITY"}
